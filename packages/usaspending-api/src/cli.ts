...
import { usaspendingPlugin } from "./plugin";
...
async function main() {
  ...
  if (filteredArgs.length === 0 || args.includes("--help")) {
    const { endpoints } = usaspendingPlugin.describe();
    console.log("Usage: usaspending-api <endpoint> [--param value ...]\n");
  ...
  const result = await dispatch([usaspendingPlugin], ["usaspending", ...filteredArgs]);
